...
public final class CCryptInputStream extends InputStream {
	...
	public CCryptInputStream(SecretKey key, InputStream source) throws IOException {
		this(key, source, true);
	}

	public CCryptInputStream(SecretKey key, InputStream source, boolean verify) throws IOException {
		this(new RijndaelEngine(256), key, source, verify);
	}

	CCryptInputStream(RijndaelEngine engine, SecretKey key, InputStream source) throws IOException {
		this(engine, key, source, false);
	}

	CCryptInputStream(RijndaelEngine engine, SecretKey key, InputStream source, boolean verify) throws IOException {
		...
		if (source.read(cipherText, 0, 32) < 32) {
			throw new IOException("Could only read " + bytesInBuffer + " bytes from ccrypt InputStream for the "
		...
		if (verifyMagic) {
			for (int i = 0; i < CCryptConstants.CCRYPT_MAGIC_BYTES.length; i++) {
				if (CCryptConstants.CCRYPT_MAGIC_BYTES[i] != plainText[i]) {
					throw new IllegalArgumentException("InputStream Magic " + "Number does not match; expected '"
							+ new String(CCryptConstants.CCRYPT_MAGIC_NUMBER) + "', found '"
							+ new String(plainText, 0, 4) + "'; wrong version of ccrypt?");
				}
			}
		}
	}

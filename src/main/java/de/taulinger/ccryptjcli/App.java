...
public class App {
	...
	public void run(String... args) {
		...
		try {
			...
			options.addOption(Option.builder("f").longOpt("force").hasArg(false).optionalArg(true)
					.desc("overwrite existing file").build());
	...
	public void encrypt(Path path, boolean force) throws URISyntaxException, IOException, InvalidKeySpecException {
		...
		final Path encryptedFile = path.getParent().resolve(path.getFileName().toString() + ".cpt");
	...
	public void decrypt(Path path, boolean force) throws URISyntaxException, IOException, InvalidKeySpecException {
		...
		final Path decryptedFile = path.getParent().resolve(path.getFileName().toString().split("\\.cpt")[0]);
	...
	Path getCurrentFolder() throws URISyntaxException {
		return Path.of(App.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParent();
	}
